...
from .models import Consultation, Medicament
...
class ConsultationSerializer(serializers.ModelSerializer):
    rendezvous_id = serializers.IntegerField(read_only=True)
    ...
    def get_medicaments(self, obj):
        ordonnance = getattr(obj, "ordonnance", None)
        if not ordonnance:
            return []
        return MedicamentSerializer(ordonnance.medicaments.all(), many=True).data
